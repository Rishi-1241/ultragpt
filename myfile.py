...
def storeBotData(username):
    try:
        ...
        if username == botid:
            ...
            purpose, whatsapp, telegram, discord, youtube, instagram, twitter, linkedin, website, username))
        cur.execute(query, (
        name, description, pic, botrole, steps, purpose, whatsapp, telegram, discord, youtube, instagram, twitter,
...
def updateBotData(username):
    try:
        ...
        if pic:
            ...
            cur.execute(query, (
            name, description, botrole, steps, purpose, whatsapp, telegram, discord, youtube, instagram, twitter,
            linkedin, website, company_info, public, botid))
        ...
        if primary:
            ...
            cur.execute(query, (
            purpose, whatsapp, telegram, discord, youtube, instagram, twitter, linkedin, website, data[1], data[1]))
        conn.commit()
        cur.close()
        return jsonify({"success": True, "message": "Bot data stored successfully."}), 200
    except Exception as e:
...
"""
    ...
    def streamResponse():
        ...
        for i in generated_text:
            ...
            elif "finish_reason" in i["choices"][0] and i["choices"][0]["finish_reason"] != None and i["choices"][0][
                "finish_reason"] != "stop" and i["choices"][0]["finish_reason"] != "timeout":
...
def getBots(username):
    ...
    try:
        ...
        conn.commit()
        cur.close()
        return jsonify({"success": True, "message": "Bots fetched successfully.",
                        "data": {"talkedBots": talkedBots, "topBots": topBots, "latestBots": latestBots}}), 200
    except Exception as e:
        print("MYSQL ERR", e)
...
def loadMorePopularBots(offset):
    ...
    try:
        ...
        try:
            return jsonify({"success": True, "message": "Bots fetched successfully.",
                            "data": bots[int(lastBotIndex) + 1:int(lastBotIndex) + 10]}), 200
        except:
            print("First try failed")
            return jsonify(
                {"success": True, "message": "Bots fetched successfully.", "data": bots[int(lastBotIndex) + 1:]}), 200
    except Exception as e:
        print("MYSQL ERR", e)
        return jsonify({"success": False, "message": "Error in fetching bots data from Database"}), 500
...
def loadMoreLatestBots(offset):
    ...
    try:
        ...
        try:
            return jsonify({"success": True, "message": "Bots fetched successfully.",
                            "data": bots[int(lastBotIndex) + 1:int(lastBotIndex) + 7]}), 200
        except:
            print("First try failed")
            return jsonify(
                {"success": True, "message": "Bots fetched successfully.", "data": bots[int(lastBotIndex) + 1:]}), 200
...
def getBotData(username, botid):
    ...
    try:
        ...
        conn.commit()
        cur.close()
        return jsonify({"success": True, "message": "Bot data fetched successfully.",
                        "data": {"bot": bot, "owner": owner, "bots": bots}}), 200
    except Exception as e:
        print("MYSQL ERR", e)
...
"""
        ...
        chatsnew = [{"name": chat[1], "username": chat[0], "lastMessage": chat[3], "count": chat[4], "image": chat[2]}
                    for chat in chats]
        conn.commit()
        cur.close()
        return jsonify({"success": True, "message": "Chats fetched successfully.", "data": chatsnew}), 200
...
token = jwt.encode({"username": "trishi1780@gmail.com"}, "h1u2m3a4n5i6z7e8")
print("Token", token)
...
def getMyBotDetails(username, botid):
    ...
    try:
        ...
        cur.close()
        return jsonify({"success": True, "message": "Bot data fetched successfully.",
                        "data": {"name": name, "pic": pic, "description": description}}), 200
    except Exception as e:
        print(e)
        ...
        if user is None:
            ...
            if check_password_hash(correctpass, password):
                ...
                return jsonify({
                    ...
                    "token": token,
                    "data": {"name": user[1], "phone": user[2], "email_id": user[3], "username": user[5],
                             "pic": user[6], "purpose": user[7], "plan": user[8], "whatsapp": user[9],
                             "youtube": user[10], "instagram": user[11], "discord": user[12], "telegram": user[13],
                             "website": user[14], "linkedin": user[19], "twitter": user[20], "favBots": user[15],
                             "pdfs": user[16], "bots": user[17], "setup": user[18], "firsttime": user[22],
                             "subscription": user[23], "verified": user[24]},
                    "bots": bots
                }), 200
            else:
...
def google_login():
    ...
    if user is None:
        ...
        email_id = data["email"]
        pic = data["picture"]
        cur.execute("INSERT INTO users (name, email_id, password, pic, refresh_token) VALUES (%s, %s, %s, %s, %s)",
                    (name, email_id, password, pic, refresh_token))
        conn.commit()
        cur.close()
        # return data except password
        token = jwt.encode({'username': email_id}, "h1u2m3a4n5i6z7e8")
        return jsonify({"success": True, "message": "Logged in successfully", "token": token,
                        "data": {"name": name, "email_id": email_id, "pic": pic}}), 200
    else:
        bots_query = "SELECT * FROM bots WHERE username=%s"
        if (user[5] != None):
            cur.execute(bots_query, (user[5],))
        ...
        return jsonify({
            ...
            "message": "Logged in successfully",
            "token": token,
            "data": {"name": user[1], "phone": user[2], "email_id": user[3], "username": user[5], "pic": user[6],
                     "purpose": user[7], "plan": user[8], "whatsapp": user[9], "youtube": user[10],
                     "instagram": user[11], "discord": user[12], "telegram": user[13], "website": user[14],
                     "favBots": user[15], "pdfs": user[16], "bots": user[17], "setup": user[18], "firsttime": user[22],
                     "subscription": user[23], "verified": user[24]},
            "bots": botsnew
        }), 200
